	...
	public Nvd3ChartsData getJsonStatsData(int year, int month, boolean currentMonth, Long userId) {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (String key : sd.getCountOfTasksPerRoleAndEffeciencyData()[i].keySet()) {
				roleBarChartDataValues[i]
						.add(new Nvd3BarChartDataValue(key, sd.getCountOfTasksPerRoleAndEffeciencyData()[i].get(key)));
			}
			...
			for (String key : sd.getCountOfTasksPerWeekAndEffeciencyData()[i].keySet()) {
				weekBarChartDataValues[i]
						.add(new Nvd3BarChartDataValue(key, sd.getCountOfTasksPerWeekAndEffeciencyData()[i].get(key)));
			}
