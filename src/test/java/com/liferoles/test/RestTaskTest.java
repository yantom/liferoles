...
public class RestTaskTest {
	...
	public void createTask() throws Exception {
		...
		given().contentType("application/json").filter(sessionFilter).body(om.writeValueAsString(t)).post("/web").then()
				.body("id", greaterThan(0)).statusCode(200);
		t.setNote("note");
		given().contentType("application/json").header(PermanentUserData.tokenHeader).body(om.writeValueAsString(t))
	...
	public void deleteTask() throws Exception {
		...
		given().header(PermanentUserData.tokenHeader).delete("/m/" + PermanentUserData.tasks.get(1).getId()).then()
				.statusCode(409);
		given().header(PermanentUserData.tokenHeader).delete("/m/" + PermanentUserData.tasks.get(2).getId()).then()
				.statusCode(204);
		try (Connection conn = ds.getConnection();
				PreparedStatement ps = conn.prepareStatement("select count(*) from task where role_id = ?");) {
			ps.setLong(1, PermanentUserData.role1.getId());
	...
	public void getInitTasks() throws Exception {
		...
		Task[] futureTasks = { PermanentUserData.tasks.get(4), PermanentUserData.tasks.get(5),
				PermanentUserData.tasks.get(6) };
	...
	public void getStats() throws Exception {
		Nvd3ChartsData expected = om.readValue(
				"{'pieChartItems':[{'key':'role1','y':3},{'key':'role2','y':2}],'barChartItemsRole':[{'key':'Earlier than planned','values':[{'x':'role1','y':0},{'x':'role2','y':0}]},{'key':'Day D','values':[{'x':'role1','y':0},{'x':'role2','y':0}]},{'key':'Within 3 days','values':[{'x':'role1','y':0},{'x':'role2','y':1}]},{'key':'Postponed','values':[{'x':'role1','y':3},{'x':'role2','y':1}]}],'barChartItemsWeek':[{'key':'Earlier than planned','values':[{'x':'Week 1','y':0},{'x':'Week 2','y':0},{'x':'Week 3','y':0},{'x':'Week 4','y':0},{'x':'Week 5','y':0}]},{'key':'Day D','values':[{'x':'Week 1','y':0},{'x':'Week 2','y':0},{'x':'Week 3','y':0},{'x':'Week 4','y':0},{'x':'Week 5','y':0}]},{'key':'Within 3 days','values':[{'x':'Week 1','y':0},{'x':'Week 2','y':0},{'x':'Week 3','y':1},{'x':'Week 4','y':0},{'x':'Week 5','y':0}]},{'key':'Postponed','values':[{'x':'Week 1','y':1},{'x':'Week 2','y':1},{'x':'Week 3','y':1},{'x':'Week 4','y':0},{'x':'Week 5','y':1}]}]}"
						.replace('\'', '"'),
				Nvd3ChartsData.class);
		Nvd3ChartsData stats = om.readValue(
				given().header(PermanentUserData.tokenHeader).get("/m/stats/2016/5").then().extract().asString(),
				Nvd3ChartsData.class);
		assertThat(stats.getPieChartItems(),
				containsInAnyOrder(expected.getPieChartItems().get(0), expected.getPieChartItems().get(1)));
		for (int i = 0; i < 4; i++) {
			assertThat(stats.getBarChartItemsRole().get(i).getValues(),
					containsInAnyOrder(expected.getBarChartItemsRole().get(i).getValues().toArray()));
			assertThat(stats.getBarChartItemsWeek().get(i).getValues(),
					containsInAnyOrder(expected.getBarChartItemsWeek().get(i).getValues().toArray()));
		}
		stats = om.readValue(given().filter(sessionFilter).get("/web/stats/2016/5").then().extract().asString(),
				Nvd3ChartsData.class);
		assertThat(stats.getPieChartItems(),
				containsInAnyOrder(expected.getPieChartItems().get(0), expected.getPieChartItems().get(1)));
		for (int i = 0; i < 4; i++) {
	...
	public void getTasksPerWeek() throws Exception {
		...
		assertThat(Arrays.asList(returned),
				containsInAnyOrder(PermanentUserData.tasks.get(3), PermanentUserData.tasks.get(4)));
	...
	public void moveTasksToBacklog() throws Exception {
		...
		try (Connection conn = ds.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("select count (*) from task where appuser_id = ? and date is null")) {
	...
	public void updateTask() throws Exception {
		...
		given().contentType("application/json").filter(sessionFilter).body(om.writeValueAsString(t))
				.put("/web/" + id.toString()).then().statusCode(204);

		// delete it
		given().header(PermanentUserData.tokenHeader).delete("/m/" + PermanentUserData.tasks.get(0).getId()).then()
				.statusCode(204);
