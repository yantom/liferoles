...
public class Utils {
	...
	public static void insertTestRoles() throws IOException, SQLException {
		...
		try (Connection conn = ds.getConnection();
				...
				PreparedStatement ps2 = conn
						.prepareStatement("insert into rolegoal (id,role_id,name,finished) values (?,?,?,?)");) {
	...
	public static void insertTestTasks() throws IOException, SQLException {
		...
		try (Connection conn = ds.getConnection(); PreparedStatement ps = conn.prepareStatement(
				"insert into task (id, name, important, role_id, appuser_id, date, time, firstDate, finished, note) values (?,?,?,?,?,?,?,?,?,?)");) {
	...
	public static void insertTestUser() throws IOException, SQLException {
		...
		try (Connection conn = ds.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("insert into appuser (id, email, password, salt) values (?,?,?,?)");
				PreparedStatement ps2 = conn
						.prepareStatement("insert into tokens (jit,appuser_id,blacklist) values (?,?,false)");) {
			ps.setLong(1, PermanentUserData.user.getId());
